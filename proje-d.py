...
def chat_server(host, port, timeout, rcvbuf, sndbuf, nonblock): 
    srv = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    srv.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    
    if rcvbuf > 0: srv.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, rcvbuf)
    if sndbuf > 0: srv.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, sndbuf)
    if timeout > 0: srv.settimeout(timeout)
    if nonblock: srv.setblocking(False)
    ...
    while True:
        try:
            ...
            srv.close()
            break
        # This part should handle socket timeouts if non-blocking is not used
        except socket.timeout:
            continue # Go back to the start of the loop to check for new connections
...
def main():
    ...
    if args.mode == "server":
     chat_server(args.host, args.port, args.timeout, args.rcvbuf, args.sndbuf, args.nonblock)
    else:
